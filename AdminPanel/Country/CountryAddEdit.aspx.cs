...
public partial class AdminPanel_Country_CountryAddEdit : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            if (Page.RouteData.Values["CountryID"] != null)
            {
                ...
                this.Page.Title = "Country Edit - Admin";

            }
            else
            {
                this.Page.Title = "Country Add - Admin";
    ...
    private void FillControls(Int32 CountryID)
    {
        if (Page.RouteData.Values["CountryID"] != null)
        {
            CountryBAL balCountry = new CountryBAL();
            CountryENT entCountry = new CountryENT();
            entCountry = balCountry.SelectPK(CountryID);
        ...
        if (Page.IsValid)
        {
            try
            {
                ...
                if (txtCountryName.Text.Trim() == String.Empty)
                    ErrorMsg += " - " + CommonMessage.ErrorRequiredField("Country Name");
                ...
                if (Page.RouteData.Values["CountryID"] != null)
                    entCountry.CountryID = Convert.ToInt32(Page.RouteData.Values["CountryID"]);
